...
class AcademicResearchAgent:
    ...
    def generate_paper_id(self, paper_data: Dict[str, Any]) -> str:
        """Generate a unique ID for a paper based on its metadata"""
        # Try to use existing paper ID if available
        if paper_data.get("paper_id"):
            return f"P{paper_data['paper_id'][:8]}"
        
        # Generate ID from title and authors
        title = paper_data.get("title", "").lower()
        authors = paper_data.get("authors", [])
        
        # Create a hash from title and first author
        import hashlib
        content = f"{title}_{authors[0] if authors else 'unknown'}"
        hash_obj = hashlib.md5(content.encode())
        return f"P{hash_obj.hexdigest()[:8].upper()}"

    def format_citation(self, paper: Dict[str, Any], paper_id: str) -> str:
        """Format a paper citation in academic style"""
    ...
    def search_semantic_scholar(self, query: str, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for paper in data.get("data", []):
                ...
                paper_data["id"] = self.generate_paper_id(paper_data)
    ...
    def generate_search_queries(self, research_topic: str, num_queries: int = 4) -> List[str]:
        ...
        The query should be standalone namely it should at least contain the topic name or one of it's related names or terms.
    ...
    def analyze_papers(self, query: str, papers: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        # Prepare papers for analysis with citations
        papers_analysis = []
        pdfs_analyzed = 0
        
        for i, paper in enumerate(papers):
            paper_id = paper.get('id', f'P{i+1:03d}')
            title = paper.get('title', 'Unknown Title')
            abstract = paper.get('abstract', '')
            authors = paper.get('authors', [])
            year = paper.get('year', 'Unknown')
            ...
            # Create citation
            citation = self.format_citation(paper, paper_id)
            ...
            if paper.get('url') and self.is_pdf_url(paper['url']):
                ...
                if title is None:
                    title = 'Unknown'
                print(f"  Downloading PDF for paper {paper_id}: {title[:50]}...")
            ...
            if not pdf_analyzed:
                ...
                for pdf_url in pdf_urls[:2]:  # Limit to first 2 PDF URLs found
                    if not pdf_analyzed:  # Only try if we haven't already analyzed a PDF
                        print(f"  Found PDF URL in content for paper {paper_id}: {pdf_url}")
...
Extract key insights and identify research gaps. For each finding, gap, or trend, reference the specific paper IDs (e.g., "P12345678 found that..." or "Research gap identified in P87654321").

Return a JSON object with:
- "key_findings": List of main findings from the papers with paper ID references (3-5 items)
- "research_gaps": List of identified research gaps with paper ID references (2-3 items)
- "trends": List of emerging trends with paper ID references (2-3 items)
- "quality_assessment": Brief assessment of paper quality and relevance"""
    ...
    def generate_research_summary(self, research_topic: str, all_papers: List[Dict[str, Any]], 
        ...
        """Generate a comprehensive academic research summary with proper citations"""
        ...
        for paper in all_papers:
            ...
            citation = self.format_citation(paper, paper_id)
            references.append(f"[{paper_id}] {citation}")
        
        # Sort references by paper ID
        references.sort()
        ...
        for source, papers in papers_by_source.items():
            ...
            for paper in papers[:3]:  # Show top 3 per source
                paper_id = paper.get('id', 'Unknown')
                papers_summary += f"- [{paper_id}] {paper['title']} ({paper.get('year', 'N/A')})\n"
...
Write a detailed academic overview (2-3 pages) that:
1. Synthesizes the current state of research with proper citations using paper IDs [P12345678]
2. Highlights key findings and contributions with specific references
...
6. Provide references to the resources but do not include links that clutter the text.
7. Make the overview structured with sections and subsections for clarity and appeal.


Use markdown format with proper academic structure. Include in-text citations using paper IDs in square brackets [P12345678] and ensure all claims are properly grounded with references."""
