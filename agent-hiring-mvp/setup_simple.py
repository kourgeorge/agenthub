#!/usr/bin/env python3
"""Simple setup script for the Agent Hiring System MVP."""
...
def install_dependencies():
    """Install Python dependencies using public PyPI."""
    ...
    # Upgrade pip first with public PyPI
    if not run_command(f"{pip_cmd} install --upgrade pip --index-url https://pypi.org/simple/"):
        return False
    
    # Install requirements with public PyPI
    if not run_command(f"{pip_cmd} install -r requirements.txt --index-url https://pypi.org/simple/"):
        return False
...
def main():
    """Main setup function."""
    logger.info("Setting up Agent Hiring System MVP (Simple Version)...")
    ...
    if not install_dependencies():
        ...
        if not run_command(f"{pip_cmd} install -r requirements.txt --index-url https://pypi.org/simple/"):
            logger.error("Alternative installation also failed")
            logger.info("Please try manual installation:")
            logger.info("1. Activate virtual environment")
            logger.info("2. Run: pip install -r requirements.txt --index-url https://pypi.org/simple/")
            sys.exit(1)
