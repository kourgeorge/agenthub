"""Admin API endpoints for managing system administration, permissions, roles, users, and billing."""
...
async def delete_permission(
    ...
    try:
        ...
        if roles_using_permission:
            ...
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Cannot delete permission '{permission.name}' as it is used by roles: {', '.join(role_names)}"
            )
        ...
        if role_data.permissions is not None:
            ...
            if role_data.permissions:
                ...
                if len(existing_permissions) != len(role_data.permissions):
                    existing_names = [p.name for p in existing_permissions]
                    missing = [p for p in role_data.permissions if p not in existing_names]
                    raise HTTPException(
                        status_code=status.HTTP_400_BAD_REQUEST,
                        detail=f"Invalid permissions: {missing}"
                    )
...
async def assign_role_to_user(
    ...
    try:
        ...
        role = db.query(Role).filter(Role.name == assignment_data.role_name).first()
        if not role:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="Role not found"
        ...
        logger.info(f"Successfully assigned role '{assignment_data.role_name}' to user {assignment_data.user_id}")
        
        return {
            "success": True,
            "message": f"Role '{assignment_data.role_name}' assigned to user successfully",
            "assignment": {
                "user_id": new_user_role.user_id,
                "role_name": role.name,
                "assigned_at": new_user_role.assigned_at.isoformat() if new_user_role.assigned_at else None,
                "assigned_by": new_user_role.assigned_by
            }
        }
...
async def remove_role_from_user(
    ...
    try:
        ...
        # Prevent removal of system roles from admin users
        if role.is_system_role and role.name == "admin":
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="Cannot remove admin role from users"
            )
